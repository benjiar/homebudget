...
export function BudgetForm({ initialData, categories, households, onSubmit, onCancel, isLoading }: BudgetFormProps) {
    ...
    const validate = (): boolean => {
        ...
        if (!formData.household_id) {
            newErrors.household_id = 'Household selection is required';
        }
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
                ...
                <select
                    ...
                    onChange={(e) => handleChange('household_id', e.target.value)}
                    disabled={isLoading || !!initialData?.household_id}
                ...
                <p className="mt-1 text-xs text-slate-500">
                    Budgets are specific to a household
                    {initialData?.household_id && ' (cannot be changed after creation)'}
                </p>
                {errors.household_id && <p className="mt-1 text-sm text-red-600">{errors.household_id}</p>}
            </div>
